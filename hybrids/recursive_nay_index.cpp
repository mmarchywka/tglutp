...
#include "recursive_nay_index.h"

// need to move the word array thing if it works...
#include "command_map.h"


typedef nay_db_driver MyClass;
...
//		push_cmd("-pfx",&Myt::m_pfx,0);
		*/
		...
		push_cmd("-format",&Myt::m_format,0);
	...
	*/				};
	...
	push_cmd("-load",&fn[0],3,0);
	push_cmd("-query",&fn[1],3,0);
	push_cmd("-out",&fn[2],3,0);
	push_cmd("-err",&fn[3],3,0);
	push_cmd("-v",&fn[4],3,0);
	push_cmd("-name",&fn[5],3,0);
	push_cmd("-hits",&fn[6],3,0);
	...
	*/
	...
	while (i<argc)
	{
		...
		else if ( strcmp(argv[i],"-continue")==0) {continue_on_error(true) ; }
		else if ( strcmp(argv[i],"-terminate")==0) {continue_on_error(false) ; }
		else if ((strcmp(argv[i],"-near")==0)&&(argc>(i+4))) 
		{ m_x_r=atof(argv[i+1]);m_y_r=atof(argv[i+2]);m_z_r=atof(argv[i+3]);  
			m_r_r=atof(argv[i+4]); i+=4;}
		/*
