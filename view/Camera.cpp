...
void Camera::transform(int x, int y, int w, int h) {

   glMatrixMode( GL_PROJECTION );
   glLoadIdentity();  // clear matrix
   int rw= _viewport_x2 - _viewport_x1 + 1;
  int rh= _viewport_y2 - _viewport_y1 + 1;
GLint viewport[]={_viewport_x1,_viewport_y1 ,rw, rh};
	
   ASSERT( _near_plane < _far_plane );
  // std::cout<<x<<" "<<y<<" "<<w<<" "<<h<<" "<<_viewport_width <<"\n";
	gluPickMatrix(x,rw-y,w,h,viewport);
   glFrustum(
      - 0.5 * _viewport_width,  0.5 * _viewport_width,    // left, right
      - 0.5 * _viewport_height, 0.5 * _viewport_height,   // bottom, top
      _near_plane, _far_plane
...
float Camera::get(int wtf)
{
	switch (wtf)
	{
		case 0: return _near_plane;
		case 1: return _far_plane; 
		//case 2: return _position;
		//case 3: return _target;
		//case 4: return _up;
		case 5: {Matrix m; m.setToLookAt(_position,_target,_up); 
		  glMultMatrixf(m.get()); return 0;  }
		}
	return 0;
	}
void Camera::clip_up(float x) {
